parse_DJ_flight_time <- function(time,
                                 force,
                                 mass,
  ...
  # Calculate the velocity at take off
  flight_time <- landing_time - take_off_time

  take_off_velocity <- TOV_from_FT(flight_time, gravity_const)
  ...
  # Correct the velocity so that take off velocity calculated from flight time is same as
  # integrated velocity
  integrated_take_off_velocity <- trace$velocity[trace$time == take_off_time]

  velocity_diff <- integrated_take_off_velocity - take_off_velocity
